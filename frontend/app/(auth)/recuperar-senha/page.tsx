  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast(data.error || 'Email não encontrado no sistema.', 'error');
      }
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-lg lg:max-w-xl bg-black/50 backdrop-blur-md border border-white/20 shadow-2xl rounded-3xl p-8 sm:p-10 lg:p-12">
        ...
        <h2 className="text-4xl lg:text-5xl font-bold text-white mb-4">Recuperar senha</h2>
        
        <p className="text-base lg:text-lg text-white mb-8 lg:mb-10">Insira seu email</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 lg:space-y-8">
          ...
          <div>
            ...
            <p className="mt-6 text-base lg:text-lg text-white/80 leading-relaxed">
              Enviaremos um link para você voltar a acessar a plataforma.
            </p>
          </div>
          ...
          <div className="text-center mt-6">
            <Link
              href="/login"
              className="text-sm lg:text-base text-white/70 hover:text-white hover:underline transition"
            >
              ← Voltar para o login
            </Link>
          </div>
