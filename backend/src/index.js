...
const atividadesMemoria = new Map();
// Rota inicial
app.get('/', (req, res) => {
    ...
    try {
        ...
        if (cursoId || turma) {
            ...
            if (cursoId && !turma) {
                ...
                where.OR.push({ tipoPublico: 'CURSO', cursoId: parseInt(cursoId) });
            }
            if (cursoId && turma) {
                // Buscar mensagens para TODOS + mensagens do CURSO + mensagens da TURMA específica
                where.OR.push({ tipoPublico: 'CURSO', cursoId: parseInt(cursoId) }, { tipoPublico: 'TURMA', cursoId: parseInt(cursoId), turma: turma });
            }
        }
        else {
...
app.put('/alterar-senha', async (req, res) => {
    try {
        ...
        const isPasswordValid = await bcrypt_1.default.compare(senhaAtual, user.password);
        if (!isPasswordValid) {
            return res.status(401).json({
